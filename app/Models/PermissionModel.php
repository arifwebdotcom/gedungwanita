...
class PermissionModel extends Model
{
    ...
    protected $validationRules = [
        'name'        => 'required|max_length[255]|is_unique[auth_permissions.name,name,id,{id}]',
    ...
    public function getPermissionsByUser($id)
    {
        return $this->db->table('auth_users_permissions')
            ->select(['auth_permissions.id', 'permission_id', 'user_id'])
            ->join('auth_permissions', 'auth_permissions.id = permission_id', 'inner')
            ->where('user_id', $id)
            ->get()
            ->getResultObject();
    ...
    public function isPermissionExists($permission, $userId)
    {
